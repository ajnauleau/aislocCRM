...
if (!$_POST) {
	...
	$get_inv_sql = "SELECT name FROM investors WHERE inv_id = '".$safe_id."'";
	$get_inv_res = mysqli_query($mysqli, $get_inv_sql)
			or die(mysqli_error($mysqli));

	while ($name_info = mysqli_fetch_array($get_inv_res)) {
	...
	$get_addresses_sql = "SELECT address FROM investors WHERE inv_id = '".$safe_id."'";
...
if (mysqli_num_rows($get_addresses_res) > 0) {
	...
	$get_pho_sql = "SELECT phone FROM investors WHERE inv_id = '".$safe_id."'";
	...
	$get_url_sql = "SELECT URL FROM investors WHERE inv_id = '".$safe_id."'";
...
$display_block .= "<p style=\"text-align:center\">
    <a href=\"updinvestor.php?sel_inv_id=".$_POST['sel_id']."\">Update Info</a> ...
