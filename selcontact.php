...
if (!$_POST) {
	...
	$get_name_sql = "SELECT CONCAT_WS(' ', f_name, l_name) as display_name
			FROM contacts WHERE con_id = '".$safe_id."'";
	...
	$get_address_sql = "SELECT address FROM contacts WHERE con_id = '".$safe_id."'";
...
if (mysqli_num_rows($get_address_res) > 0) {
	...
	$get_pho_sql = "SELECT phone FROM contacts WHERE con_id = '".$safe_id."'";
	...
	$get_email_sql = "SELECT email FROM contacts WHERE con_id = '".$safe_id."'";
...
$display_block .= "<p style=\"text-align:center\">
    <a href=\"updcontact.php?sel_con_id=".$_POST['sel_id']."\">Update Info</a> ...
